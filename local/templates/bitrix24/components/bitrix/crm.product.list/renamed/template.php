...
{?>
    ...
    $result = explode("/", $page);
    $section = CIBlockSection::GetByID($result[4]);
    $newRes = $section->Fetch();
    switch($result[4]){
        case 340:{
            ?>
            <div class= "btrx-button_adn_text_inforamtion">
                <a href="/marketing-materials/index.php" class="ui-btn ui-btn-primary ui-btn-icon-add crm-btn-toolbar-add btrx-button-ebash" title="Перейти в формированию товара в калькуляторе">Перейти к заказу маркетинговых материалов</a>
                <div class="btrx-text-products-information">
                    В данном списке появляются сформированные заказы маркетинговых материалов.<br>
        ...
        case 341:{
            ?>
            <div class= "btrx-button_adn_text_inforamtion">
                <a href="/calculator/index.php" class="ui-btn ui-btn-primary ui-btn-icon-add crm-btn-toolbar-add btrx-button-ebash" title="Перейти в формированию товара в калькуляторе">Перейти к формированию товара в калькуляторе</a>
                <div class="btrx-text-products-information">
                    В данном списке появляются сформированные товары в калькуляторе.<br>
                    Товары в данном списке доступны только для просмотра, без возможности редактирования.
                </div>
            </div>
            <?
