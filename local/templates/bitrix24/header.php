...
if (in_array(17, $USER->GetUserGroupArray()))
{
	$bodyClass .= " addstyle-distributor addstyle-allpartners";
}
if (in_array(20, $USER->GetUserGroupArray()))
{
	$bodyClass .= " addstyle-diler addstyle-allpartners";
}
if (in_array(21, $USER->GetUserGroupArray()))
{
	$bodyClass .= " addstyle-subdiler addstyle-allpartners";
}
if (in_array(22, $USER->GetUserGroupArray()))
{
	$bodyClass .= " addstyle_User_TO addstyle-allpartners";
}
?>
<title><? if (!$isCompositeMode || $isIndexPage) $APPLICATION->ShowTitle()?></title>


    <?if(CSite::InDir('/map/index.php')) {
        $bodyClass .= " map-page";
        $APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/style.css');
        $APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH."/script/JS.js", true);?>
        <!--<link rel="stylesheet" type="text/css" href="<?=SITE_TEMPLATE_PATH?>/css/style_map.css">-->
    <?}?>

    <?if(CSite::InDir('/marketing-materials/'))
        $bodyClass .= " mm-page";
    ?>

    <?if(CSite::InDir('/instruction/index.php'))
        $bodyClass .= " instructions-page";
    ?>
	...
	<?if(CSite::InDir('/instruction/index.php')) {
		...
		$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/instructions.css');
	}
	$APPLICATION->SetAdditionalCSS(SITE_TEMPLATE_PATH.'/css/privateCabinet.css');
	?>
...
?>
							...
							<tr class="bx-layout-inner-inner-top-row">
								<td class="bx-layout-inner-inner-cont">
									<div class="page-header">
										...
										$APPLICATION->IncludeComponent("bitrix:menu", "marketing-materials", Array(
	"ROOT_MENU_TYPE" => "left",	// Тип меню для первого уровня
		...
		"CHILD_MENU_TYPE" => "marketing-materials",	// Тип меню для остальных уровней
	...
	false
										...
										<?if(CSite::InDir('/marketing-materials/index.php')) {?>
											<div class="pagetitle-wrap marketing-materials">
										<?}else {?>
								...
								<td class="bx-layout-inner-inner-cont">
									...
									<div id="workarea">
										<?if($APPLICATION->GetProperty("HIDE_SIDEBAR", "N") != "Y"):
											...
											?></div>
										<?endif?>
										<?if(CSite::InDir('/crm/product/') || CSite::InDir('/calculator/index.php')) {?>
											<div id="workarea-content" class="btrx-no-paddings">
										<?}else {?>
											<div id="workarea-content">
										<? } ?>
												<div class="workarea-content-paddings">
